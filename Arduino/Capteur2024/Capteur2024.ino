...
void loop() {
  ...
  ecranOLED.setCursor(0,0);              // Position du curseur

  int Etat_Switch = digitalRead(Switch);
  if(Etat_Switch == LOW){             // Assure le calibrage de la résistance R2

    ecranOLED.println("...");
    ecranOLED.println("Calibration");
    ecranOLED.println("en cours...");
    ecranOLED.display(); 
    //calibration(); la fonction crash pour le moment
    ecranOLED.clearDisplay();
    delay(2000);
  }
  ...
  ecranOLED.print("Valeur : ");          // Affichage du texte
 // ecranOLED.print(Tension_sortie);       // Affichage de la valeur
